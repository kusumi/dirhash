...
var (
	...
	alg_sum_list_2_str    = "A"
	alg_sum_list_2_repeat = 1000000
)

func Test_getByteHash(t *testing.T) {
	for _, x := range alg_sum_list_1 {
		written, sum, err := getByteHash([]byte{}, x.hash_algo)
		if err != nil {
			t.Error(x)
		}
		if written != 0 {
		...
		if hex_sum := getHexSum(sum); hex_sum != x.hex_sum {
			t.Error(x)
		}
	}

	s := bytes.Repeat([]byte(alg_sum_list_2_str), alg_sum_list_2_repeat)
	for _, x := range alg_sum_list_2 {
		written, sum, err := getByteHash(s, x.hash_algo)
		if err != nil {
			t.Error(x)
		}
		if written != uint64(alg_sum_list_2_repeat) {
			t.Error(written)
		}
		if hex_sum := getHexSum(sum); hex_sum != x.hex_sum {
			t.Error(x)
...
func Test_getStringHash(t *testing.T) {
	for _, x := range alg_sum_list_1 {
		written, sum, err := getStringHash("", x.hash_algo)
		if err != nil {
			t.Error(x)
		}
		if written != 0 {
	...
	s := strings.Repeat(alg_sum_list_2_str, alg_sum_list_2_repeat)
	for _, x := range alg_sum_list_2 {
		written, sum, err := getStringHash(s, x.hash_algo)
		if err != nil {
			t.Error(x)
		}
		if written != uint64(alg_sum_list_2_repeat) {
